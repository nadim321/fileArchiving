...
// Check if image file is a actual image or fake image
        ...
        if ($uploadOk == 0) {
            ...
            if (move_uploaded_file($target_tmp, $target_file)) {
                ...
                // query for get story for logged in user
                $sql = "SELECT * FROM thesis where id!=$thesis_id and status = 1 and deleted = 0 order by id asc";
                $stmt = $dbh->prepare($sql);
                $result = $stmt->execute();
                ...
                if($titleMatchigPercent > 50 || $abstractMatchigPercent > 50){
                    ...
                    // query for new story upload
                    $sql = "update thesis set title=? , abstract=?2, teacher_id=?3 where id = ?4";
                    $stmt = $dbh->prepare($sql);
                    // set value to query
                    $params = [$title, $abstract ,$teacherId,  $teacherId];
                    //           print_r($params);
                    $result = $stmt->execute($params);
                }
